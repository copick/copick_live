...
from copick_live.utils.figure_utils import blank_fig, draw_gallery
from copick_live.utils.local_dataset import get_local_dataset
from copick_live.config import get_config
from dash import html, Input, Output, callback, State, ALL, MATCH, ctx, dcc, no_update
    ...
    return dbc.Card(
        [
            dbc.CardHeader(
                [
                    DashIconify(
                        icon="noto-v1:trophy", width=25, style={"margin": "5px"}
                    ),
...
@callback(
    Output("run-dt", "data"), Input("tomogram-index", "data"), prevent_initial_call=True
)
def load_tomogram_run(tomogram_index):
    ...
    if tomogram_index is not None:
        # takes 18s for VPN
        t1 = time.time()
        get_copick_dataset().load_curr_run(run_name=tomogram_index, sort_by_score=True)
        # takes 0.2s
        t2 = time.time()
...
def update_analysis(
    ...
    if tomogram_index:
        # time.sleep(7)
        # particle_dict = {k: k for k in sorted(set(get_copick_dataset().dt['pickable_object_name']))}
        if at == "tab-1":
            time.sleep(2)
            particle_dict = {
                k: k
                for k in sorted(set(get_copick_dataset().dt["pickable_object_name"]))
            }
            df = pd.DataFrame.from_dict(get_copick_dataset().dt)
            fig1 = px.scatter_3d(
                df,
                x="x",
                y="y",
                z="z",
                color="pickable_object_name",
                symbol="user_id",
                size="size",
                opacity=0.5,
            )
            ...
            # new_particle = None
            if pressed_key in [
                str(i + 1) for i in range(len(get_local_dataset()._im_dataset["name"]))
            ]:
                new_particle = get_local_dataset()._im_dataset["name"][
                    int(pressed_key) - 1
                ]
            elif pressed_key == "s":
                new_particle = kbn

            get_copick_dataset().new_user_id(user_id=copicklive_username)
            if (
                "display-row" in changed_id or "display-col" in changed_id
            ) or particle in get_copick_dataset().points_per_obj:
                if len(get_copick_dataset().points_per_obj[particle]) % (nrow * ncol):
                    slider_max = len(get_copick_dataset().points_per_obj[particle]) // (
                        nrow * ncol
                    )
                else:
                    slider_max = (
                        len(get_copick_dataset().points_per_obj[particle])
                        // (nrow * ncol)
                        - 1
                    )
            ...
            positions = [
                i
                for i in range(
                    slider_value * nrow * ncol,
                    min(
                        (slider_value + 1) * nrow * ncol,
                        len(get_copick_dataset().points_per_obj[particle]),
                    ),
                )
            ]
            ...
            dim_z, dim_y, dim_x = get_copick_dataset().tomogram.shape
            ...
            selected = [
                i for i, v in enumerate(thumbnail_image_select_value) if v % 2 == 1
            ]
            selected_point_ids = [positions[i] for i in selected]
            if "accept-bttn" in changed_id or pressed_key == "a":
                get_copick_dataset().handle_accept_batch(selected_point_ids, particle)
            elif "reject-bttn" in changed_id or pressed_key == "d":
                get_copick_dataset().handle_reject_batch(selected_point_ids, particle)
            elif "assign-bttn" in changed_id or pressed_key == "s":
                get_copick_dataset().handle_assign_batch(
                    selected_point_ids, particle, new_particle
                )
            ...
            #     slider_value += 1
            #     positions = [i for i in range(slider_value*nrow*ncol, min((slider_value+1)*nrow*ncol, len(get_copick_dataset().points_per_obj[particle])))]
...
def select_thumbnail(
    value, select_clicks, unselect_clicks, slider_value, nrow, ncol, particle, comp_id
):
    """
    ...
    selected = [
        get_copick_dataset().picked_points_mask[
            get_copick_dataset().points_per_obj[particle][i][0]
        ]
        for i in positions
    ]
    ...
    if value is None or (
        ctx.triggered[0]["prop_id"] == "unselect-all-bttn.n_clicks" and color == ""
    ):
        return ""
    if value % 2 == 1:
        return "primary"
...
def download_txt(n_clicks):
    ...
    if counter_file_path:
        ...
        task_contents = "\n".join(
            local_dataset.dirs[
                counter["start"] : counter["start"] + counter["tasks_per_person"]
            ]
        )
        print(task_contents)
...
def update_results(n):
    ...
    return (
        fig,
        dbc.ListGroup(
            [candidate_list(i, j) for i, j in candidates.items()], flush=True
        ),
        dbc.ListGroup(
            [ranking_list(i, len(j)) for i, j in num_per_person_ordered.items()],
            numbered=True,
        ),
        [label],
        bar_val,
...
def update_compositions(n):
    ...
    for tomogram, ps in tomograms.items():
        ...
        bttn = html.Button(
            id={"type": "tomogram-eval-bttn", "index": tomogram},
            className="fa fa-search",
            style=roundbutton,
        )
        progress_list.append(
            dbc.ListGroupItem(
                children=[dbc.Row([tomogram, bttn]), dbc.Progress(progress)],
